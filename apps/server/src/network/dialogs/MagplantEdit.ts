...
export class MagplantEdit {
  ...
  public async execute(): Promise<void> {
    ...
    // Item-picker dialogs return the chosen value directly in the field
    // (e.g. magplant_choose_item=2) WITHOUT setting buttonClicked, so we
    // must detect this case before the switch.
    const hasItemPickerResult =
      this.action.magplant_choose_item !== undefined && btn === "";

    if (hasItemPickerResult) {
      ...
      const itemInfo = this.base.items.metadata.items.get(chosenID.toString());
